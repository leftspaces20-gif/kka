...
app.get('/card', async (req, res) => {
  ...
  try {
    ...
    // fallback: Discord logosu gösterilecek
  }

  // Instruction içinde bold kısmı <strong> ile sar
  const instructionHTML = instruction.replace(
    bold,
    `<strong>${bold}</strong>`
  );

  // Avatar içeriği: resim varsa <img>, yoksa Discord SVG ikonu
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .card::after {
    ...
    inset: 0;
    background:
      radial-gradient(ellipse 260px 180px at 10% 50%, rgba(88,101,242,0.22) 0%, transparent 70%),
      radial-gradient(ellipse 140px 120px at 95% 100%, rgba(88,101,242,0.1) 0%, transparent 70%);
    pointer-events: none;
    z-index: 0;
  }
  .card-body {
  ...
  .discord-icon-wrap {
    ...
    justify-content: center;
    box-shadow:
      0 0 0 1.5px rgba(109,124,246,0.5),
      0 6px 28px rgba(88,101,242,0.65),
  ...
  .v-divider {
    ...
    background: linear-gradient(180deg,
      transparent 0%,
      rgba(88,101,242,0.4) 30%,
      rgba(88,101,242,0.4) 70%,
      transparent 100%);
  }
  .content-section {
    flex: 1;
  ...
  .word {
    ...
    line-height: 1;
    letter-spacing: -2px;
    background: linear-gradient(125deg, #ffffff 20%, #9da4ff 100%);
...
<div class="card">
  ...
  try {
    ...
    const png = await page.screenshot({
      type: 'png',
      clip: { x: 0, y: 0, width: 500, height: 128 },
      omitBackground: true,
    });
